...
vhost() {
    ...
    # Send a request to the root domain with a fake host header to measure response size
    local baseline_size=$(curl -s -o /dev/null -H "Host: falsebaseline.$1" $url -w '%{size_download}')

    local wordlist="$seclists_dir/Discovery/DNS/bitquark-subdomains-top100000.txt"

    if [ -z "$baseline_size" ] || [ "$baseline_size" -eq 0 ]; then
        echo "[-] Could not grab baseline. Running standard ffuf without size filter..."
        ffuf -H "Host: FUZZ.$1" -u $url -w "$wordlist" "${@:2}" -c
    else
        echo "[+] Dynamic Filter Found! Filtering out size: $baseline_size bytes"
        # Run ffuf using the domain directly, forcing the Host header variations
        ffuf -H "Host: FUZZ.$1" -u $url -w "$wordlist" "${@:2}" -fs "$baseline_size" -c
    fi
    #ffuf -H "Host: FUZZ.$1" -u $url -w "$wordlist" "${@:2}"
}

# Optimized VHOST Fuzzing with Auto-Size Suppression
# Optimized VHOST Fuzzing - Domain Only
ffvhost() {
    if [ -z "$1" ]; then
        echo "Usage: ffvhost <DOMAIN>"
        echo "Example: ffvhost machine.htb"
        return 1
    fi

    local domain=$1
    local wordlist="/usr/share/seclists/Discovery/Web-Content/vhosts-to-test.txt"

    echo "[*] Calculating baseline false-positive size for http://$domain..."
    
    # Send a request to the root domain with a fake host header to measure response size
    local baseline_size=$(curl -s -o /dev/null -H "Host: falsebaseline.$domain" "http://$domain" -w '%{size_download}')

    if [ -z "$baseline_size" ] || [ "$baseline_size" -eq 0 ]; then
        echo "[-] Could not grab baseline. Running standard ffuf without size filter..."
        ffuf -w "$wordlist" -u "http://$domain" -H "Host: FUZZ.$domain" -c
    else
        echo "[+] Dynamic Filter Found! Filtering out size: $baseline_size bytes"
        # Run ffuf using the domain directly, forcing the Host header variations
        ffuf -w "$wordlist" -u "http://$domain" -H "Host: FUZZ.$domain" -fs "$baseline_size" -c
    fi
}
