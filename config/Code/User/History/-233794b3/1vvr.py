...
def search_nvd_by_keyword(keyword: str, exact: bool = False, limit: int = 15) -> list[dict]:
    """Search NVD CVE database by keyword (mirrors the NVD web search).

    Args:
        keyword:  The search term, e.g. 'FreePBX' or 'FreePBX 16.0.40.7'.
        exact:    If True, use keywordExactMatch for a phrase search.
        limit:    Maximum number of CVEs to return.

    Returns a list of dicts with keys:
        cve_id, score, severity, published, description, url
    """
    results = []
    try:
        params = {
            "keywordSearch":  keyword,
            "resultsPerPage": min(limit * 2, 40),  # fetch more so sort has enough to work with
            "startIndex":     0,
            # NOTE: NVD v2 API does NOT support sortOrder=score — we sort client-side
        }
        if exact:
    ...
    # Sort by score descending (N/A goes to end)
    def _score_key(r):
        s = r.get("score", 0)
        try:
            return float(s)
...
def display_nvd_keyword_results(results: list[dict], keyword: str):
    ...
    table = Table(
        title=f"[bold white]NVD results for keyword: [cyan]{keyword}[/cyan][/bold white]",
        box=box.ROUNDED,
        border_style="red",
    ...
    # Full NVD URLs (never truncated)
    for i, r in enumerate(results, 1):
        console.print(f"  [{i:02d}] [bright_blue]{r['url']}[/bright_blue]")
...
def display_summary(all_results: dict, query: str, elapsed: float):
    ...
    if rows:
        ...
        console.print()
        for i, (source, title, url) in enumerate(rows[:30], 1):
            console.print(
...
def run_search(query: str, token: str = None, no_nuclei: bool = False):
    ...
    if primary_cve:
        ...
        # We kick this off now (synchronously, before the parallel block)
        # so results appear at the top before GitHub output.
        software_name = extract_software_name(query) or query
        console.print(Rule("[bold red]>> NVD CVE Keyword Search[/bold red]", style="red"))
        with console.status(
            f"[bold red]Searching NIST NVD for CVEs matching '{software_name}'...[/bold red]"
        ):
            # Try exact-match first (phrase search), fall back to broad
            nvd_keyword_results = search_nvd_by_keyword(software_name, exact=True)
            if not nvd_keyword_results:
                nvd_keyword_results = search_nvd_by_keyword(software_name, exact=False)
            # If a version was given, also try the full string
            if software_name != query:
                full_results = search_nvd_by_keyword(query, exact=False, limit=10)
                # Merge without duplicates
                existing_ids = {r["cve_id"] for r in nvd_keyword_results}
                for r in full_results:
                    if r["cve_id"] not in existing_ids:
                        nvd_keyword_results.append(r)
                        existing_ids.add(r["cve_id"])
                # Re-sort by score
                def _score_key(r):
                    try:
                        ...
                        return 0.0
                nvd_keyword_results.sort(key=_score_key, reverse=True)

        display_nvd_keyword_results(nvd_keyword_results, software_name)
        console.print()

        # Auto-expand the top-scoring CVE with full details
        if nvd_keyword_results:
            top = nvd_keyword_results[0]
            ...
            if top_cve and top_cve.startswith("CVE-"):
                ...
                console.print()
                # Expose top CVE for nuclei search in the parallel block below
                primary_cve = top_cve
