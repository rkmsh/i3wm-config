...
def search_exploits_for_cves(
    ...
    searches on GitHub, ExploitDB, PacketStorm, and Sploitus in parallel.
    ...
    for cve in cves:
        ...
        tasks.append((cve, "PacketStorm", search_packetstorm,      cve))
...
def display_nvd_keyword_results(results: list[dict], keyword: str):
    ...
    render_w = max(console.width - 1, 80)
...
def run_search(query: str, token: str = None, no_nuclei: bool = False):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        if _cve_targets:
            ...
            cve_exploit_future = executor.submit(
                search_exploits_for_cves, _cve_targets, token, 5
            )
    ...
    # PacketStorm: merge keyword + CVE-specific
    ps_all  = list(all_results.get("PacketStorm", []))
    ps_seen = {r.get("url") for r in ps_all}
    for cve_id, hits in cve_exploit_results.items():
        for h in hits:
            if h.get("source") == "PacketStorm" and h.get("url") not in ps_seen:
    ...
    all_results["PacketStorm"]= ps_all
    ...
    for cve_id, hits in cve_exploit_results.items():
        for h in hits:
            ...
            if src not in ("GitHub Repos", "GitHub Code", "GitHub Topics",
                           "ExploitDB", "PacketStorm", "Sploitus",
