...
def search_packetstorm(query: str, vulners_key: str = None, add_search_link: bool = False) -> list[dict]:
    ...
    if api_key:
        ...
        try:
            ...
            # Append order:published to sort by publication date descending (latest first) on Vulners
            vulners_query = f'"{cve_upper}" order:published' if cve_upper else f"{query} order:published"
            exploit_hits = vapi.search.search_exploits(vulners_query, limit=MAX_RESULTS)
            ...
            if cve_upper and len(results) < 5:
                bulletin_hits = vapi.search.search_bulletins(
                    f'"{cve_upper}" order:published', limit=10
                )
...
def search_sploitus(query: str) -> list[dict]:
    ...
    try:
        payload = {
            ...
            "query":  query,
            "offset": 0,
            "sort":   "date",    # Sort by date on the server to get the latest results
